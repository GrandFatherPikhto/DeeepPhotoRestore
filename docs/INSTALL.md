# Обучение нейросети на основе PyTorch для восстановления изображений
...
```bash
python3 -m torch.distributed.run --nproc_per_node=1 NAFNet/basicsr/train.py -opt options/train/Tanahen/train_NAFNet_JPEG_1060.yml --launcher pytorch
```
...
Обученные модели будут находится в директории

```bash
./NAFNet/experiments
```

Например, если в настроечном файле (.yml) имя проекта (`name`) будет `NAFNet-JPEG-1060`, директория с моделями будет называться `NAFNet-JPEG-1060`

Можно запускать скрипт `inference.py`. Он по обученным моделям воссоздаст изображение из зашумленного.

## Installation
Настроить config в `.ssh`:
```ini
Host github-tanahen
  HostName github.com
  User git
...
[https://github.com/rybkinatalia1-wq/DeepPhotoRestore.git](https://github.com/rybkinatalia1-wq/DeepPhotoRestore.git)
