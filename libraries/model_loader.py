...
class NAFNetRGBWrapper(nn.Module):
    ...
    """
    def __init__(self, original_model, nafnet_channels=3, out_channels=3):
        super().__init__()
        self.net = original_model
        
        # 🎯 Исправление: берем каналы напрямую из переданного параметра, 
        # не полагаясь на внутренние имена слоев оригинальной модели
        if nafnet_channels != out_channels:
            print(f"⚙️ [Враппер] Добавлен адаптер каналов: {nafnet_channels} -> {out_channels}")
...
def load_network(opt, device):
    ...
    if 'arch_file' in net_opt and net_opt['arch_file']:
        ...
        # 🎯 Исправление: убрали висящие запятые, которые нечаянно превращали списки в кортежи
        ...
        # Оборачиваем модель, передавая количество её каналов (на выходе NAFNet каналов столько же, сколько на входе)
        model = NAFNetRGBWrapper(raw_model, nafnet_channels=in_channels, out_channels=out_channels).to(device)
